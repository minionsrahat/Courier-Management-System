...
class Action
{
	...
	{
		extract($_POST);
		$qry = $this->db->query("SELECT *,concat(firstname,' ',lastname) as name FROM admin_users where email = '" . $email . "' and password = '" . md5($password) . "'  ");
		if ($qry->num_rows > 0) {
	...
	function delete_parcel_request()
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM parcel_requests where id = $id");
		if ($delete) {
			return 1;
		}
	}

	function accept_parcel_request()
	{
		extract($_POST);
		foreach ($price as $k => $v) {
			$data = "";
			foreach ($_POST as $key => $val) {
				if (!in_array($key, array('id', 'weight', 'height', 'width', 'length', 'price','ref')) && !is_numeric($key)) {
					if (empty($data)) {
						$data .= " $key='$val' ";
					} else {
						$data .= ", $key='$val' ";
		...
		$delete = $this->db->query("UPDATE `parcel_requests` SET `status`=10 where id = $id");
		if (isset($save) && isset($ids)) {
			// return json_encode(array('ids'=>$ids,'status'=>1));
			return 1;
		}
	...
	function get_report()
	{
		...
		while ($row = $get->fetch_assoc()) {
			$row['sfname'] = ucwords($row['sfname']);
			$row['rfname'] = ucwords($row['rfname']);
